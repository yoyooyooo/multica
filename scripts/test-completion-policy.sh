...
case "$project" in
  cc_multica_upstream_main_20260808_*) ;;
  *) echo "compose project must start with cc_multica_upstream_main_20260808_" >&2; exit 2 ;;
esac
...
if [[ "$root" != "$expected_root" ]]; then
  echo "completion harness must run from the admitted upstream-main-20260808 worktree" >&2
  exit 2
fi

compose_file="$root/docker-compose.context-continuity-test.yml"
port=55449
database=multica_cc_upstream_main_20260808
user=multica_cc_upstream_main_20260808
password=multica_cc_upstream_main_20260808_local_only
...
assert env.get("POSTGRES_USER")=="multica_cc_upstream_main_20260808"
assert len(ports)==1
p=ports[0]
