...
case "$project" in
  cc_multica_v049_*) ;;
  *) echo "compose project must start with cc_multica_v049_" >&2; exit 2 ;;
esac
...
if [[ "$root" != "$expected_root" ]]; then
  echo "completion harness must run from the admitted v0.4.9 worktree" >&2
  exit 2
fi

compose_file="$root/docker-compose.context-continuity-test.yml"
port=55449
database=multica_cc_v049
user=multica_cc_v049
password=multica_cc_v049_local_only
...
assert env.get("POSTGRES_USER")=="multica_cc_v049"
assert len(ports)==1
p=ports[0]
