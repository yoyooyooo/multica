...
cp scripts/selfhost-wait.sh scripts/migrate-selfhost-uploads.sh scripts/copy-legacy-uploads.sh "$recipe_dir/scripts/"
...
build_config="$(GOPROXY='https://goproxy.example,direct' docker compose --env-file "$tmp_env" -f docker-compose.selfhost.yml -f docker-compose.selfhost.build.yml config)"
...
if bash scripts/copy-legacy-uploads.sh "$conflict_source" "$conflict_target" >/dev/null 2>&1; then
  echo "legacy uploads migration accepted a conflicting bind-owned file"
  exit 1
fi
...
if grep -Eq 'JWT_SECRET:|MULTICA_EXTERNAL_PR_SERVICE_TOKEN:' <<<"$helm_config"; then
  echo "Helm rendered secret values instead of existingSecret references"
  exit 1
fi
