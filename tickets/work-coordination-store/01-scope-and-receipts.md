...
顶层在Cobra parse前只为`multica coordination`命令预扫描output mode，同时接受`--output json|table`和`--output=json|table`。Flag必须出现在`coordination`之后、argument terminator `--`之前，可位于合法nested command之前或之后。Pre-scanner与coordination Cobra command tree共享单一value-taking flag metadata并按token/arity推进；被known flag消费的value token即使恰为`--output`或`--output=json`也不得二次识别，`--flag=...`右侧与普通value中的`--output` substring均不得递归/substring匹配。Unknown或无法唯一判定flag/value归属的序列不猜测output mode，而以默认JSON renderer返回local validation error、exit 5且零HTTP。真正的output flag全argv只允许出现0或1次；重复（相同值或冲突值）、缺值、非法值均在零HTTP请求前返回`coordination_invalid_payload`、exit 5。Flag/positional/local validation/server error都由同一个renderer输出；JSON失败时stdout为空、stderr恰好一个stable envelope JSON value（允许末尾换行），即使`--debug`也不得追加prose/stack/raw body。Table失败保留安全prose。V1顶层execute helper与子进程response-classifier测试只以scope POST的`coordination_idempotency_conflict`和三类delete的`coordination_delete_blocked`证明strict ProductError/exit 6，并证明其余三种future 409 code放在任一V1 route时均fallback/exit 1；独立纯单元测试直接对五种已构造的known 409 `ProductError`验证全局exit mapper均返回6。V2/V3在新增routes时补各自exact method/route/code classifier fixtures；V4补inspect GET + `coordination_revision_conflict`正向fixture及该route其他known 409的negative fallback fixtures，并与V5验证完整五项end-to-end矩阵。V1仍覆盖unknown route、legacy/string/unknown/status-mismatch 409、两种output flag形态及nested-command前后位置、known value-taking flag的separate/equals-form value等于或包含`--output[=...]`时不误判、unknown/ambiguous flag-value序列default-JSON fail closed、重复相同/冲突、缺值、非法值、零请求、stdout空/stderr单JSON、debug无prose，以及exit 1/3/4/5/6。
...
7. CLI exact request、zero-request validation；V1 response classifier只允许scope POST的`coordination_idempotency_conflict`和三类delete的`coordination_delete_blocked`进入strict ProductError/exit 6，其余三种future 409 code放在任一V1 route时均fallback/exit 1；独立纯单元测试证明五种已构造known 409 ProductError的global mapper均exit 6；后续V2/V3/V4只能按本节增量扩exact route/code classifier；legacy/status-mismatch 409 exit 1；`--output json`/`--output=json`、table、nested位置、`--`、known flag的separate/equals-form values恰为或包含`--output[=...]`时不得误判、unknown/ambiguous flag-value序列default-JSON/zero-request，以及missing value/empty value/invalid value/duplicate/conflicting values矩阵；JSON failure stdout空/stderr单value，debug无prose，top-level exit 1/3/4/5/6；
...
12. guard触发时task/Autopilot DB与cache/S3/metrics/reconciliation/event均未变化；测试证明pinned connection、session lock、entity row locks、guard、pre-delete task/token/Autopilot DB operations、Workspace既有row/chat locks及final delete共用一个qtx；Single/Workspace任一步失败都由`Delete`返回typed error，effects为空且handler显式`Finish(false)`后全部rollback；handler无法取得raw qtx；
