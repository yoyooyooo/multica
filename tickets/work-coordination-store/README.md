...
Coordination默认output为JSON。顶层在Cobra parse前扫描`coordination`后的完整argv，识别`--output json|table`与`--output=json|table`，包括nested subcommand前后由Cobra接受的位置，并尊重`--` terminator。Scanner必须是token/flag-arity aware parser，并与coordination Cobra command tree共享同一份value-taking flag metadata；已被前一个known flag按Cobra语义消费的value token不得再次识别为output flag，即使其值恰为`--output`或`--output=json`，`--flag=...`右侧也不得递归扫描，普通value中包含`--output` substring同样不匹配。Unknown或无法唯一判定flag/value归属的序列不得猜测output mode，而以默认JSON renderer返回local validation error、exit 5且零HTTP。真正的output flag全argv只允许出现一次；missing value/empty value/invalid value/duplicate/conflicting values均在零HTTP请求前返回`coordination_invalid_payload`/exit 5，并以默认JSON renderer输出。恰有一个有效`table`才使用safe prose；其余coordination失败stdout为空、stderr恰一个JSON envelope，即使`--debug`也不追加prose/stack/raw body。
