...
12. **删除fail closed且无TOCTOU**：所有Store mutations在自身transaction内取得统一workspace transaction-level advisory lock。Issue单删、BatchDeleteIssues与Workspace删除在任何DB mutation或外部副作用前取得pinned connection、冲突session lock与qtx，持锁检查scope/dependency/record/typed-ref guard；不得把瞬时`Check*DeletionAllowed`当保护。Guard通过后，必须pre-delete的task/token/Autopilot DB mutations与entity delete共享qtx；commit成功后先verified unlock并release/discard connection，随后才调用cache/S3/metrics/reconciliation/event等现有best-effort effects。Receipt history/reference本身不独立阻塞删除；replay仍重验current authority/resource。Guard拒绝零mutation；qtx失败整体rollback；V1不声称post-release effects bounded、可靠投递、可观测失败、自动重试或与DB原子。完整协议见下节。
...
- Delete path在任何mutation/外部副作用前从现有pgx pool取得pinned `*pgxpool.Conn`，先在该session调用`pg_advisory_lock(namespace,workspace_key)`，再在同一connection开始并持续持有`pgx.Tx`。具体`IssueDeletionHandle`/`WorkspaceDeletionHandle`在内部按UUID锁Issue rows，或先锁Workspace row再按UUID锁chat-session rows，然后执行Store guard；guard通过后，其at-most-once `Delete`在同一qtx完成task cancellation DB updates、task-token cleanup、Autopilot failure updates、既有membership teardown与最终Issue/Workspace delete，并把immutable post-commit payload保留在handle内部，handler不得取得raw qtx或提前执行effects。`CommitDB`确认成功后，`ReleaseAfterCommit`立即用独立5秒cleanup context执行同session `pg_advisory_unlock`并验证true，成功才`Release`；unlock false/error或connection异常必须`Hijack`并close/discard。只有connection已terminal released/discarded后，handle才把payload交给handler调用现有metrics/reconciliation/cache/S3/event seams。Process crash/connection close由PostgreSQL释放session lock。
- 单次操作只允许一个workspace。固定顺序为`workspace advisory lock → entity rows按UUID byte order → scope/resource rows按UUID byte order`。BatchDeleteIssues锁前只按route workspace做UUID语法解析、去重与raw-byte排序，不读取actual targets；取得该workspace的一次session lock/qtx后，再以workspace-scoped query加载并锁定仍存在的actual target set，missing/foreign维持skip/no-leak，随后一次性guard。Actual targets为all-or-nothing；禁止per-target savepoint/continue。不得嵌套或排序多个workspace locks。
- 任一statement或commit返回PostgreSQL `40001`（serialization failure）或`40P01`（deadlock）时，整次单删/Batch/Workspace删除失败：不得skip target、不得在failed tx或同一handle继续、不得运行post-commit effects、V1不得自动retry。可确认未commit时以独立cleanup context rollback→verified unlock；任何commit outcome unknown、rollback/unlock状态不明或transport/connection异常都必须Hijack+close/discard，返回不含SQLSTATE/原始错误的`coordination_internal`，且禁止自动retry/finalizer。
- 单Issue、BatchDeleteIssues、Workspace删除的session lock/qtx/guard均早于任何task/Autopilot/cache/event mutation。Guard通过后只允许同qtx pre-delete DB operations；Workspace不能先移除membership或invalidate cache。DB commit确认成功后必须先完成verified unlock/release-or-discard，再调用post-release effects；同步listener、S3或其他adapter因此不能延长或重入该session lock。
- 所有guard都在持锁期间执行；禁止“check后unlock，再做mutation/delete”。Store mutation持有冲突xact lock并在写前复核entity，故并发结果只能是Store写先成功而delete被guard，或delete先完成而Store写因entity不存在失败。Guard rejection及qtx rollback保证pre-delete DB mutation为零/回滚。当前event bus与storage seams不提供统一context/error contract，因此V1只验证post-release调用尝试与锁已释放，不声明effect成功、30秒总deadline、逐effect幂等、typed retry debt、可靠投递或自动修复；这些属于future lifecycle/outbox。
...
Server envelope固定为`{"error":{"code":"coordination_*","message":"...","details":{...}}}`；`details`可省略，出现时按code严格allowlist且不得含SQL、constraint或输入原文。CLI只在coordination endpoint的JSON content type、strict known envelope与上表status/code精确匹配时构造`ProductError`。五个409 code——`coordination_capacity_exceeded`、`coordination_revision_conflict`、`coordination_idempotency_conflict`、`coordination_dependency_scope_conflict`、`coordination_delete_blocked`——全部exit 6；legacy string/body、unknown code或status/code mismatch的409继续现有safe fallback/exit 1。

Coordination默认output为JSON。顶层在Cobra parse前扫描`coordination`后的完整argv，识别`--output json|table`与`--output=json|table`，包括nested subcommand前后由Cobra接受的位置，并尊重`--` terminator。只允许出现一次；missing value、空值、非法值、同值重复或json/table冲突均在零HTTP请求前返回`coordination_invalid_payload`/exit 5，并以默认JSON renderer输出。恰有一个有效`table`才使用safe prose；其余coordination失败stdout为空、stderr恰一个JSON envelope，即使`--debug`也不追加prose/stack/raw body。
