...
- Delete path在任何mutation/外部副作用前从现有pgx pool取得pinned `*pgxpool.Conn`，先在该session调用`pg_advisory_lock(namespace,workspace_key)`，再在同一connection开始并持续持有`pgx.Tx`。该qtx按UUID锁entity rows后执行Store guard；guard通过后，必须在entity delete前完成的task cancellation DB updates、task-token cleanup、Autopilot failure updates、Workspace既有row/chat-session locks与最终Issue/Workspace delete全部使用同一qtx。Commit成功后才执行agent reconciliation、metrics、cache invalidation、S3 cleanup与task/Issue/Workspace event publish等不可回滚动作；最后在同一session `pg_advisory_unlock`并验证true，成功才`Release`。Guard/pre-delete/delete失败走rollback→unlock；unlock false/error或transaction/connection异常必须`Hijack`并close/discard connection。Process crash/connection close由PostgreSQL释放session lock。
