...
- Delete path在任何副作用前从现有pgx pool取得pinned `*pgxpool.Conn`，先在该session调用`pg_advisory_lock(namespace,workspace_key)`，再在同一connection开始并持续持有`pgx.Tx`。该qtx按UUID锁entity rows后执行Store guard；Issue/Workspace最终DB delete、Workspace既有row/chat-session locks也必须使用同一qtx。`Finish`先commit/rollback qtx，再在同一session调用`pg_advisory_unlock`并验证true；成功才`Release`。Unlock false/error或transaction/connection异常时必须`Hijack`并close/discard connection，禁止带锁或状态不明的connection回pool。Process crash/connection close由PostgreSQL释放session lock。
