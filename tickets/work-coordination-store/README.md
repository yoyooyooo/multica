...
- Initial analytical base：`5e8661b8efb30c0728fb515ea7fa9a9b631a0c02`；最近一次against-code readiness复核使用source head `225d166975aaadc32e9517c6295e6b192761bdef`。
- 在`225d166975aaadc32e9517c6295e6b192761bdef`观察到的migration ceiling仍为`201`；V1及每个后续slice开始前必须在上一片accepted exact head重新扫描`server/migrations/*.up.sql`，再原子预留无碰撞prefix range。
...
- Delete path在任何mutation/外部副作用前从现有pgx pool取得pinned `*pgxpool.Conn`，先在该session调用`pg_advisory_lock(namespace,workspace_key)`，再在同一connection开始并持续持有`pgx.Tx`。具体`IssueDeletionHandle`/`WorkspaceDeletionHandle`在内部按UUID锁Issue rows，或先锁Workspace row再按UUID锁chat-session rows，然后执行Store guard；guard通过后，其at-most-once `Delete`在同一qtx完成task cancellation DB updates、task-token cleanup、Autopilot failure updates、既有membership teardown与最终Issue/Workspace delete，并返回immutable bounded post-commit payload，handler不得取得raw qtx。`CommitDB`成功后才执行agent reconciliation、metrics、cache invalidation、S3 cleanup与task/Issue/Workspace event publish等不可回滚动作；最后在同一session `pg_advisory_unlock`并验证true，成功才`Release`。Handle是at-most-once state machine；cleanup使用独立5秒bounded context。Advisory lock acquisition error视为session状态不明；guard/pre-delete/delete失败走rollback→unlock；unlock false/error、qtx状态不明或connection异常都必须`Hijack`并close/discard，绝不能`Release`。Process crash/connection close由PostgreSQL释放session lock。
...
- 单Issue、BatchDeleteIssues、Workspace删除的session lock/qtx/guard均早于任何task/Autopilot/cache/event mutation。Guard通过后只允许同qtx pre-delete DB operations；cache/S3/metrics/reconciliation/event等必须等待commit。Session lock持续到post-commit动作已完成或记录失败债务后再释放；Workspace不能先移除membership或invalidate cache。
- 所有guard都在持锁期间执行；禁止“check后unlock，再做mutation/delete”。Store mutation持有冲突xact lock并在写前复核entity，故并发结果只能是Store写先成功而delete被guard，或delete先完成而Store写因entity不存在失败。Guard rejection及qtx rollback保证pre-delete DB mutation为零/回滚；commit后的外部动作失败只记录可重试债务，不虚构DB rollback。
