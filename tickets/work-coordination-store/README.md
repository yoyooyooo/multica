...
12. **删除fail closed且无TOCTOU**：所有Store mutations在自身transaction内取得统一workspace transaction-level advisory lock。Issue单删、BatchDeleteIssues与Workspace删除则在任何cache/task/Autopilot/event副作用前从pool Acquire pinned `*pgxpool.Conn`，先在该connection取得冲突的session-level lock，再begin并持有同connection上的`pgx.Tx`；所有entity row lock、scope/dependency/record/typed issue-ref guard与最终entity DB row delete都只经绑定该tx的同一qtx执行。Receipt history/reference本身不构成独立删除阻塞；replay仍须按receipt规则重新验证当前authority和resource。Guard拒绝时保证零副作用；guard通过后仅保证不会新增受guard保护的Store soft-ref orphan，不承诺既有delete流程后续失败所产生的task/Autopilot/event债可回滚。完整协议见下节。
...
- Delete path在副作用前从pool Acquire pinned `*pgxpool.Conn`，先在该connection调用`pg_advisory_lock(namespace, workspace_key)`，成功后才begin并持有同connection上的`pgx.Tx`。返回handle只允许绑定该tx的同一qtx执行entity row lock、Store guard及最终Issue/Workspace row delete，禁止换回pool或另起transaction。
- 单次操作只允许一个workspace。固定顺序为`workspace advisory lock → begin pgx.Tx → entity rows按UUID byte order → scope/resource rows按UUID byte order → 最终entity row delete`；BatchDeleteIssues先解析并确认所有实际目标属于同一workspace，再取得一次lock，并在同一qtx按UUID byte order一次性锁定及guard全部目标。Workspace删除既有workspace row lock与chat-session locks也必须并入该qtx。不得嵌套或排序多个workspace locks。
- `Finish`必须先commit或rollback该`pgx.Tx`，再在同一session调用`pg_advisory_unlock`并验证返回true，之后才可归还connection；unlock返回false或报错时必须close/discard该connection，禁止带锁归还pool。Process crash或connection close由PostgreSQL释放session lock。
- 单Issue、BatchDeleteIssues、Workspace删除从Acquire、session lock、begin、row lock、guard到最终delete与Finish，全程均早于cache invalidation、task cancellation、Autopilot mutation和event publish。Workspace删除不能先移除membership或cache。
