...
Server envelope固定为`{"error":{"code":"coordination_*","message":"...","details":{...}}}`；`details`可省略，出现时按code严格allowlist且不得含SQL、constraint或输入原文。CLI只在allowlisted method/route/code组合同时满足JSON content type、strict known envelope与上表status/code精确匹配时构造`ProductError`。V1 allowlist包含三条scope routes，以及Issue单删、BatchDeleteIssues、Workspace删除仅返回`coordination_delete_blocked`的guard分支；V2/V3只随新增coordination routes显式扩展。五个409 code——`coordination_capacity_exceeded`、`coordination_revision_conflict`、`coordination_idempotency_conflict`、`coordination_dependency_scope_conflict`、`coordination_delete_blocked`——在allowlisted组合中全部exit 6；其他route、legacy string/body、unknown code/envelope或status/code mismatch的409继续现有safe fallback/exit 1。
