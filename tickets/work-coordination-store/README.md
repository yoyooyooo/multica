...
- 任一statement或commit返回PostgreSQL `40001`（serialization failure）或`40P01`（deadlock）时，整次单删/Batch/Workspace删除失败：不得skip target、不得在failed tx或同一handle继续、不得运行post-commit effects、V1不得自动retry。可确认未commit时以独立cleanup context执行verified rollback→verified unlock；任何commit outcome unknown、rollback/unlock状态不明或transport/connection异常都必须Hijack+close/discard，返回不含SQLSTATE/原始错误的`coordination_internal`，且禁止自动retry/finalizer。
