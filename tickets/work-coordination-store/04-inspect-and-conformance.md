...
JSON原样保留scope revision、完整active dependencies、完整open blockers、receipt page和`next_receipt_cursor`。Table只做人类显示。API/CLI均调用public service method；无raw DB shortcut或message-substring error分类。V4增量使用`coordination_internal`处理invariant破坏；全部code/HTTP/CLI exit引用[README Stable wire error SSoT](README.md#stable-wire-error-ssot)。V4 response classifier只新增`GET /api/coordination/scopes/{scopeId}/inspect` + `coordination_revision_conflict`这一409组合并映射ProductError/exit 6；其他known 409 code、legacy body或status mismatch出现在inspect route时仍须fallback/exit 1。

V4增加顶层CLI conformance test：成功JSON stdout只有一个value；失败JSON stderr只有一个error envelope且exit code匹配stable product code，无main追加prose。Inspect stale-revision正向fixture必须命中上述exact route/code组合；同route上的其余四种known 409、legacy/string body及status mismatch逐一证明安全fallback/exit 1。
...
对scope root、`coordination_dependency` endpoints、record字段、create/resolution relation refs和workspace逐类注入单删、BatchDeleteIssues、Workspace删除。Guard拒绝发生在任何task/Autopilot DB mutation或cache/S3/metrics/reconciliation/event前；Acquire内部先verified rollback再verified unlock并release/discard，任一状态不明都Hijack+close/discard且禁止pool release，并且不返回handle。Receipt history/reference单独存在时不阻塞删除，旧receipt replay因current authority/resource revalidation失败。并发Ensure/Add/Append与三类delete证明无新orphan。无受guard保护引用的成功删除验证：Acquire pinned session lock→begin qtx/UUID row locks/Store guard→同qtx task-token-Autopilot-Workspace dependent DB cleanup→final entity delete→at-most-once `Finish(commit bool)`以独立bounded cleanup context完成commit/rollback、verified unlock及release/discard→仅当commit结果明确成功且`Finish`完整成功后执行typed effects；effects期间绝不持session lock、不静默截断，cardinality/内存/时延上界为`not_claimed`。Single/Workspace保持atomic。Batch保持invalid/not-found/inaccessible/foreign skip、resolved UUID dedupe、empty `deleted=0`与`Finish(true)`明确成功后的unique-row计数；一次lock/qtx加载锁定全部actual targets并一次guard，guard conflict整批零写；guard全过后逐target用handle内部savepoint，只有V1 allowlist中的`entity_delete/23503`且rollback-to/release成功、tx valid才返回`skipped_recoverable`，其余phase/unknown/transaction/savepoint error整批Abort。Commit outcome unknown时discard、返回`coordination_internal`、无effects/自动retry/rollback声称。Acquire返回后handler立即安装由`finishStarted`保护的deferred `Finish(false)`；任何显式Finish调用前先置flag并disarm，调用一旦开始无论返回success/error都不得再次调用，Finish内部自行release-or-discard terminalize。Blocking/reentrant listener及S3 fake证明effects开始前session lock已terminal；当前void/no-error adapter只验证调用尝试，不虚构effect成功。Effects失败只走既有error/log/operator debt，不虚构DB rollback。
