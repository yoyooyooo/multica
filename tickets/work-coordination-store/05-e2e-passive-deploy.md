...
5. 同一actor的独立B process重放A key，得到原receipt、revision不变；
6. 不同actor复用key得到idempotency conflict；同actor在membership/task authority失效后不能用old key读取旧receipt；
7. B resolve blocker→`r3`，dependency仍active；
8. A resolve dependency→`r4`，最终active edge/open blocker为空；
9. stale revision、changed-hash、cycle/self、cross-workspace、other-scope pair、task越权均stable code且零部分写；
10. 并发反向edge最多一方成功。
...
```bash
make sqlc
(cd server && go test ./internal/migrations ./cmd/migrate ./pkg/db/...)
(cd server && go test ./internal/service ./internal/handler ./internal/middleware ./internal/cli ./cmd/multica)
...
- sqlc第二次生成无drift；
- migration覆盖fresh、legacy upgrade与空Store down/up；
- DB tests明确实际执行；
...
6. **Compatibility**：旧CLI对新server无破坏；新CLI遇旧server安全报unsupported/not-found，不误写metadata。
...
- 最终active dependency/open blocker为空；receipt refs仍可分页读取；
- legacy dependency计数/内容不变；
- Store tracer rows作为evidence保留。由于首期deletion guard，临时workspace不得被强删；lifecycle cleanup另开ticket。
...
V5完成要求：aggregate source flow、focused/race/full/build/check、fresh review、exact-head CI、deployment gate、明确批准、live two-client/task auth/no-side-effect/deletion-guard proof、exact version readback和rollback artifact全部通过。
