...
9. `coordination_revision_conflict`、`coordination_idempotency_conflict`、`coordination_cycle`、`coordination_self_dependency`、`coordination_cross_workspace`、`coordination_dependency_scope_conflict`、`coordination_forbidden`与`coordination_invalid_payload`均按README SSoT返回且零部分写；独立CLI matrix证明`coordination_capacity_exceeded`、`coordination_revision_conflict`、`coordination_idempotency_conflict`、`coordination_dependency_scope_conflict`、`coordination_delete_blocked`五个409均exit 6，legacy/status-mismatch 409仍exit 1，并覆盖两种`--output`语法与missing value/empty value/invalid value/duplicate/conflicting values；
10. 并发反向edge最多一方成功；Ensure/Add/Append分别与单删、BatchDeleteIssues、Workspace删除race均不产生新orphan。三类delete按同一phase执行：Acquire pinned `*pgxpool.Conn`→session lock→begin `pgx.Tx`→UUID Issue locks或Workspace row+chat locks→guard；guard通过后在同qtx执行必须pre-delete的task/token/Autopilot DB mutations、membership teardown及final delete；commit明确成功后立即verified unlock/release-or-discard，再调用best-effort metrics/reconciliation/cache/S3/events。Guard或qtx失败零外部副作用并整体rollback；statement/commit `40001`、`40P01`整批失败且不continue/retry/finalize，commit outcome unknown不finalize并discard；blocking/reentrant listener不能持有session lock。当前void/no-error effects只证明调用尝试，不声明deadline、成功、typed debt、可靠投递或自动修复；
