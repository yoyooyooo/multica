...
10. 并发反向edge最多一方成功；Ensure/Add/Append分别与单删、BatchDeleteIssues、Workspace删除race均不产生新orphan；
11. 第1000条active dependency/open blocker可写，第1001条返回`coordination_capacity_exceeded`且零写入；inspect单响应完整返回active/open facts，receipt refs固定100条按`(created_at DESC,id DESC)`游标分页。
...
- inspect在硬上限内始终完整返回active dependency/open blocker；最终两组为空，receipt refs仍按固定100条、`(created_at DESC,id DESC)`及upper-bound+revision cursor分页读取；
