...
9. `coordination_revision_conflict`、`coordination_idempotency_conflict`、`coordination_cycle`、`coordination_self_dependency`、`coordination_cross_workspace`、`coordination_dependency_scope_conflict`、`coordination_forbidden`与`coordination_invalid_payload`均按README SSoT返回且零部分写；CLI复用V1唯一SSoT，并在V2/V3已随新增routes落地各自exact method/route/code allowlist、V4已加入inspect GET + `coordination_revision_conflict`唯一新增组合后，验证五个strict envelope/status匹配的coordination 409逐code构造`ProductError`并exit 6，legacy/string/unknown/status-mismatch 409 exit 1；goldens、两种output flag形态/位置、known flag values恰为或包含`--output[=...]`时不误判、unknown/ambiguous flag-value序列fail closed、重复/冲突/缺值/非法值零HTTP、stdout空/stderr单JSON及debug无prose均通过；
10. 并发反向edge最多一方成功；Ensure/Add/Append分别与单删、BatchDeleteIssues、Workspace删除race均不产生新orphan。三类delete按同一phase执行：Acquire pinned `*pgxpool.Conn`→session lock→begin `pgx.Tx`→UUID Issue locks或Workspace row+chat locks→guard→同qtx task/token/Autopilot/Workspace dependent DB cleanup及final delete→at-most-once `Finish(commit bool)`以独立bounded cleanup context完成commit/rollback、verified unlock及release或Hijack+close/discard；只有commit结果明确成功且`Finish`完整成功后运行immutable typed effects，effects期间绝不持session lock、不静默截断，cardinality/内存/时延上界为`not_claimed`。Acquire/guard错误内部先verified rollback再verified unlock/release-or-discard，任一状态不明都Hijack+close/discard且禁止pool release，不返回半初始化handle；handler无raw qtx，取得handle后立即安装由`finishStarted`保护的deferred `Finish(false)`；任何显式Finish调用前先置flag并disarm，调用一旦开始无论返回success/error都不得再次调用，Finish内部自行release-or-discard terminalize。Single/Workspace atomic。Batch保持invalid/not-found/inaccessible/foreign skip、resolved UUID dedupe、empty `deleted=0`与`Finish(true)`明确成功后的unique-row计数；一次lock/qtx加载锁定全部actual targets并一次guard，guard conflict整批零写；guard全过后逐target用handle内部savepoint，只有V1 allowlist中的`entity_delete/23503`且rollback-to/release成功、tx valid才返回`skipped_recoverable`，其余phase/unknown/transaction/savepoint error整批Abort。Commit outcome unknown时discard、返回`coordination_internal`、无effects/自动retry/rollback声称。Blocking/reentrant listener及S3 fake证明effects开始前session lock已terminal；当前void/no-error adapter只验证调用尝试，不虚构effect成功。Effects失败只走既有error/log/operator debt，不虚构DB rollback；
