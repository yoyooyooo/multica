...
6. 不同actor复用key得到`coordination_idempotency_conflict`；同actor在membership/task authority失效后不能用old key读取旧receipt；
...
10. 并发反向edge最多一方成功；Ensure/Add/Append分别与单删、BatchDeleteIssues、Workspace删除race均不产生新orphan。三类delete都必须在cache/task/Autopilot/event前从pool Acquire pinned `*pgxpool.Conn`，同connection依次执行session advisory lock、begin `pgx.Tx`、按UUID锁entity rows及guard，并只允许tx-bound qtx最终delete；Workspace既有row lock及chat-session locks并入该qtx；`Finish`先commit/rollback，再session unlock并验证true，false/error时close/discard；
11. 两类容量独立计数、互不占用配额：active dependencies从第1至第1000条均可写，第1001条dependency返回`coordination_capacity_exceeded`且dependency/revision/receipt不变；open blockers另行从第1至第1000条均可写，第1001条blocker返回同code且record/ref/revision/receipt不变。分别覆盖两组1000/1001 cases后，inspect单响应完整返回各自上限内的active/open facts，receipt refs固定100条按`receipt_ordinal DESC`与ordinal upper-bound cursor分页。
...
- verbose coordination DB command在DB不可用或任一test skip时non-zero，并输出实际执行的migration/integration test names；
