...
9. `coordination_revision_conflict`、`coordination_idempotency_conflict`、`coordination_cycle`、`coordination_self_dependency`、`coordination_cross_workspace`、`coordination_dependency_scope_conflict`、`coordination_forbidden`与`coordination_invalid_payload`均按README SSoT返回且零部分写；CLI strict `ProductError`、coordination 409 exit6、legacy/string 409 exit1及JSON single-value stderr均通过；
10. 并发反向edge最多一方成功；Ensure/Add/Append分别与单删、BatchDeleteIssues、Workspace删除race均不产生新orphan。三类delete按同一phase执行：Acquire pinned `*pgxpool.Conn`→session lock→begin `pgx.Tx`→UUID entity locks→guard→同qtx task/token/Autopilot/Workspace DB cleanup及final delete→commit/rollback→verified unlock/release；at-most-once `Finish`使用独立bounded cleanup context；只有成功`Finish`后运行typed bounded effects，effects期间不持session lock，异常Hijack+close/discard。Single/Workspace atomic；Batch按resolved UUID去重并skip invalid/not-found/inaccessible，empty actual set返回`deleted=0`，非guard单target失败仅用同qtx savepoint继续且effects只含成功target，任一guard conflict整批零写，最终commit失败整体rollback，`deleted`为最终commit的实际唯一row数。Effects失败只走既有error/log/operator debt，不虚构DB rollback或可靠投递；
...
- DB-required机械语义只引用V1冻结的`./scripts/verify-work-coordination-db-tests.sh`；它必须证明`internal/migrations`、`cmd/migrate`、`internal/service`、`internal/handler`各有matching test实际run+pass，并使DB不可用、skip、零匹配或`Skipping tests:` non-zero；
