...
`make sqlc`后的tracked diff与untracked porcelain assertions必须同时为空，任一非空立即使V2 gate失败；`git diff --check`不能替代。V1建立的DB-required harness扩充V2 manifest：DB不可用、任何skip或任一required package缺`TestWorkCoordination*` pass evidence都必须non-zero。
