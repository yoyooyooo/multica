    ...
    entries: [
      {
        version: "0.3.31",
        date: "2026-06-26",
        title: "跨工作区未读提醒、Composio 工具集底座、更顺手的编辑器",
        changes: [],
        features: [
          "工作区切换器里，凡是其他工作区有未读 Inbox 消息时，旁边都会亮起品牌色小圆点。无需切换工作区也能一眼看到最新动态——背后通过一次账号级汇总请求实现，新通知到来或被读掉时小圆点会实时点亮或熄灭",
          "Multica 正式接入 Composio SDK 底座，为即将上线的第三方工具集对接打好基础——账号连接、工具调用以及带签名的 Webhook 回调均已底层就绪",
          "现在可以同时在多个 Git Worktree 中并行启动桌面端开发环境——每个 Worktree 会分到独立的渲染端口与独立的应用实例，两份本地代码也不会再为同一个窗口打架",
          "中文文档首页新增可点击播放的中文介绍视频，同时新的 VideoEmbed 组件已注册到文档站，可在任意文档页面复用",
        ],
        improvements: [
          "贡献者文档里明确说明桌面端开发命令会按 Worktree 自动隔离，与新的运行方式保持一致",
        ],
        fixes: [
          "在 Issue 编辑器列表里按 Tab，现在能稳定地缩进所选条目，光标也不会跑出列表——多项选中后按 Tab，会把首项作为锚点，后续项目嵌套缩进；当某项无法再缩进时，Tab 也不会再把焦点带出编辑器",
          "通过 @ 提及或在评论里要求小队 Leader 组织协作时，Leader 现在会带上完整的小队 Briefing 启动，而不是独自闷头干活；同时回复时继承父评论的 @小队 提及也不会再触发 Leader 第二次启动",
          "Issue 和评论中的代码块在页面其他区域刷新时，所选文本不会再被打断——智能体输出实时刷新过程中也能正常复制代码片段",
          "把 Issue 直接交给某个具体智能体时，运行确认弹窗会立即展开 Handoff 备注框，不再卡在「检查中…」状态",
          "Issue 未读小红点的统计逻辑与你实际看到的 Inbox 一致——同一个 Issue 的重复通知会先合并再计数，已经清空的 Inbox 不会再让切换器小圆点继续亮着",
          "编辑 Issue 评论时，保存按钮会显示明确的加载状态，直到改动真正提交完成",
          "搜索结果在后端接口结构调整后能够稳定加载，不会再因为格式不匹配而失败",
          "自托管部署在缺少 Docker Compose v2 时会立刻给出明确的安装提示，不再抛出旧版命令行那种让人摸不着头脑的 'unknown shorthand flag' 错误",
        ],
      },
      {
        version: "0.3.30",
