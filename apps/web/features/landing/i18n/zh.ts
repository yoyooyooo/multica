    ...
    entries: [
      {
        version: "0.3.29",
        date: "2026-06-24",
        title: "自动任务、飞书协作和运行灰度更可靠",
        changes: [],
        features: [
          "飞书对话现在接入统一的协作通道基础，消息处理更一致，也为后续接入更多聊天平台做好准备",
          "功能灰度能力覆盖到应用和守护进程行为，团队可以更稳地分阶段开放高风险改动",
          "智能体读取很长的 Issue 讨论时，可以把已解决的讨论折叠到关键结论，减少无效上下文",
          "飞书用户可以用 `/new` 开启新会话；Lark WebSocket 连接也可以走指定代理",
        ],
        improvements: [
          "定时自动任务更可靠：漏跑、重试和多个执行者同时处理时，会更稳定地收敛到预期的一次运行",
          "智能体运行开场说明可以在灰度开关后变得更精简，减少噪音，同时保留完整回退路径",
          "运行时服务商文档已同步当前支持情况，补充 Qoder、CodeBuddy 指引，并清理过时的 Gemini CLI 信息",
          "项目仓库设置里的检出版本会跟随本地智能体工作生效，避免拿到错误分支",
        ],
        fixes: [
          "父 Issue 下的子 Issue 现在会按创建顺序稳定展示",
          "Issue 里的同源附件预览可以正常打开",
          "@ 提及选择器现在会提交当前高亮的人或 Issue，不会因为搜索结果重排而选错",
          "取消后的聊天草稿删除后不会在切换页面回来时重新出现",
          "自动任务冷启动、Issue 顶部智能体状态和 Antigravity 服务商错误提示更准确",
        ],
      },
      {
        version: "0.3.28",
