      ...
      entries: [
        {
          version: "0.3.33",
          date: "2026-06-30",
          title: "Autopilot のアクセス管理、Slack の履歴バックフィルと入力リアクション、スキル パッケージのアーカイブ取り込み",
          changes: [],
          features: [
            "Autopilot に明確な書き込み権限レイヤーが入りました。編集・実行・Webhook の更新は作成者またはワークスペースの Owner/Admin だけが行え、Webhook トークンも他のメンバーには表示されません。詳細ページに「アクセス管理」が追加され、ワークスペース全体の管理権限を渡すことなく、特定のメンバーに 1 つの Autopilot の書き込み権限だけを委譲できます。",
            "Slack チャンネルの過去のやり取りを、統一された chat history プルで Multica にバックフィルできるようになりました。エージェントがチャンネルに加わった時点で、これまでの会話の流れをそのまま把握できます。",
            "Slack でエージェントが返信を準備している間、ユーザーのメッセージに 👀 のリアクションが付き、応答完了・失敗・タスクが起動しなかった場合のいずれでも確実に外れます。",
            "スキル パッケージをローカルの .skill / .zip アーカイブから取り込めるようになりました。URL 取り込みと同じ衝突戦略・サイズ上限・予約ファイルのルールが適用され、multica skill import --file <パス> で利用できます。",
            "Agents ページがモバイルに最適化され、スマートフォンからも快適に閲覧・操作できます。",
          ],
          improvements: [
            "コメントのルーティング カスケードを全面的に書き直しました。親リンクからの @ メンション、エージェント署名の返信、スクワッド リーダーへのフォールバックが、十分にテストされた 1 本の経路にまとまり、エージェント署名コメントでスクワッド リーダーへのフォールバックが失われていた回帰も併せて修正しました。",
            "multica issue 系のコマンドは短い UUID プレフィックスを受け付けなくなりました（旧来のリゾルバは大きなワークスペースでタイムアウトしがちでした）。Issue Key（MUL-123 など）または完全な UUID を指定してください。古い形式を入力した場合は明確な案内エラーが返ります。",
            "ロケール バンドルから、実際にはレンダリングされない `_one` 複数形キーを 117 件削除し、再発を防ぐパリティ チェックを追加しました。組み込みランタイム一覧の Gemini を実稼働している CodeBuddy に差し替えました。",
            "セルフホストの事前チェックは新しい Docker Compose CLI プラグインを許容しつつ、旧来の Docker Compose v1 は引き続き弾きます。",
          ],
          fixes: [
            "WebSocket 再接続後、ローカル デーモンは進行中のタスクとワークスペース状態を即座にサーバーと突き合わせるようになり、最長 30 秒のティッカーを待つことがなくなりました。（コミュニティ コントリビューター xiawiie、#4665）",
            "Antigravity エージェントが「ターンを完了したのに何も出力しない」ケースでも、デーモンが実行ログから返信内容を回収するようになり、会話が空白になりません。",
            "CLIENT SETNAME を拒否するマネージド Redis でサーバーが起動できなかった問題を修正しました。（コミュニティ コントリビューター MeloMei）",
            "エージェント活動のホバー カードのヘッダーが「実行中 N タスク」と表記され、ワークスペース側に表示される重複しないエージェント数と食い違うことがなくなりました。",
          ],
        },
        {
          version: "0.3.32",
