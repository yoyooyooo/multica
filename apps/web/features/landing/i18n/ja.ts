      ...
      entries: [
        {
          version: "0.3.31",
          date: "2026-06-26",
          title: "ワークスペース横断の未読表示、Composio ツールキット基盤、より使いやすいエディター",
          changes: [],
          features: [
            "ワークスペース切替メニューで、いまアクティブではない他のワークスペースに未読のインボックスがあるとブランドカラーのドットが表示されます。切り替えなくても他ワークスペースの新着に気づけ、アカウント単位の集計で取得しているため、新着の到着や既読化に合わせてリアルタイムに点いたり消えたりします。",
            "Multica にスタンドアロンの Composio SDK 基盤が組み込まれ、これから提供するサードパーティ ツールキット連携の土台ができました。接続アカウントの作成、ツール実行、署名付き Webhook 受信までが内部で一通りつながっています。",
            "複数の Git ワークトリーでデスクトップ開発環境を並列に立ち上げても衝突しなくなりました。ワークトリーごとに専用のレンダラー ポートと独立したアプリ インスタンスが割り当てられ、同じウィンドウの取り合いが起きません。",
            "中国語ドキュメントのトップに、クリックすると再生できる中国語イントロ動画を追加しました。新しい VideoEmbed ビルディング ブロックは他のドキュメント ページでも再利用できます。",
          ],
          improvements: [
            "コントリビューター ドキュメントを更新し、デスクトップ開発コマンドがワークトリーごとに自動で隔離されることを明示しました。",
          ],
          fixes: [
            "Issue エディター内のリストで Tab を押すと、選択した項目が安定して字下げされ、カーソルがリストの外に飛ばなくなりました。複数項目を選択して Tab を押すと、先頭を起点に残りがその下にネストされ、それ以上字下げできない位置でも Tab がエディター外にフォーカスを移すことはありません。",
            "コメントの @メンションや返信でスクワッド リーダーに依頼すると、リーダーはスクワッドのブリーフィングを携えて起動し、独力で片付けようとしなくなります。親コメントの @スクワッド メンションを引き継いだ返信が、もう一度リーダーの実行をトリガーすることもありません。",
            "Issue やコメント内のコード ブロックで選択したテキストが、画面の別領域が再描画されても解除されなくなり、エージェントの出力がライブで流れているときでもそのままコピーできます。",
            "Issue を特定のエージェントに直接アサインするとき、実行確認のハンドオフ メモ欄が「確認中…」で止まらず、すぐに開きます。",
            "Issue インボックスの未読表示が、実際に画面で見えるインボックスと一致するように整いました。同じ Issue の重複通知を先にまとめてから数えるので、空のインボックスに切替メニューのドットが残り続けることがありません。",
            "Issue のコメント編集時、保存ボタンに明確なローディング表示が出るようになり、変更が反映されるまでの状態が一目でわかります。",
            "バックエンド API の応答フォーマット変更後も、検索結果が安定して読み込まれます。",
            "セルフホスト環境で Docker Compose v2 が見つからないときは、その時点で意味のあるインストール案内とともに停止します。旧コマンドラインが返していた 'unknown shorthand flag' という分かりづらいエラーはもう出ません。",
          ],
        },
        {
          version: "0.3.30",
