      ...
      entries: [
        {
          version: "0.3.33",
          date: "2026-06-30",
          title: "Autopilot 액세스 관리, Slack 히스토리 백필과 입력 반응, 스킬 번들 아카이브 가져오기",
          changes: [],
          features: [
            "Autopilot에 명확한 쓰기 권한 계층이 도입되었습니다. 편집·실행·웹훅 갱신은 작성자 또는 워크스페이스 Owner/Admin만 수행할 수 있고, 웹훅 토큰도 그 외 멤버에게는 노출되지 않습니다. 상세 페이지에는 '액세스 관리'가 추가되어, 워크스페이스 전체 관리 권한을 주지 않고도 특정 멤버에게 단일 Autopilot의 쓰기 권한을 위임할 수 있습니다.",
            "Slack 채널의 과거 대화를 통합된 chat history 풀로 Multica에 백필할 수 있습니다. 에이전트가 채널에 합류한 순간부터 이전 대화 맥락을 그대로 알 수 있습니다.",
            "Slack에서 에이전트가 응답을 준비하는 동안 사용자 메시지에 👀 반응이 표시되며, 응답 완료·실패·작업이 트리거되지 않은 경우 모두에서 안정적으로 제거됩니다.",
            "스킬 번들을 로컬 .skill / .zip 아카이브에서 가져올 수 있습니다. URL 가져오기와 동일한 충돌 정책·용량 제한·예약 파일 규칙이 적용되며 multica skill import --file <경로> 로 사용합니다.",
            "Agents 페이지가 모바일에 맞게 다듬어져 스마트폰에서도 편하게 보고 조작할 수 있습니다.",
          ],
          improvements: [
            "댓글 라우팅 캐스케이드를 전면 다시 작성했습니다. 부모 체인 @멘션, 에이전트 서명 답글, 스쿼드 리더 폴백이 모두 충분히 테스트된 하나의 경로로 모여 동작이 더 예측 가능해졌고, 에이전트 서명 댓글에서 스쿼드 리더 폴백이 빠지던 회귀도 함께 복구했습니다.",
            "multica issue 계열 명령은 더 이상 짧은 UUID 접두사를 받지 않습니다(이전 리졸버는 큰 워크스페이스에서 타임아웃되곤 했습니다). Issue Key(MUL-123) 또는 전체 UUID를 사용하세요. 옛 형식을 입력하면 명확한 안내 오류가 표시됩니다.",
            "실제로 렌더링되지 않는 `_one` 복수 키 117개를 로케일 번들에서 제거하고 재발을 막는 패리티 검증을 추가했으며, 내장 런타임 목록의 Gemini를 실제 사용되는 CodeBuddy로 교체했습니다.",
            "셀프호스트 사전 검사는 최신 Docker Compose CLI 플러그인을 허용하면서 구버전 Docker Compose v1은 계속 거부합니다.",
          ],
          fixes: [
            "WebSocket 재연결 후 로컬 데몬이 진행 중인 작업과 워크스페이스 상태를 서버와 즉시 동기화하도록 바뀌어, 최대 30초의 티커를 기다리지 않습니다. (커뮤니티 기여자 xiawiie, #4665)",
            "Antigravity 에이전트가 '턴을 마쳤지만 아무 것도 출력하지 않는' 경우에도 데몬이 실행 기록에서 응답을 복원해, 대화가 비어 보이지 않습니다.",
            "CLIENT SETNAME을 거부하는 매니지드 Redis에서 서버가 기동되지 않던 문제를 수정했습니다. (커뮤니티 기여자 MeloMei)",
            "에이전트 활동 호버 카드의 헤더가 '실행 중 N 작업'으로 표시되어, 워크스페이스 칩에 나오는 중복 제거된 에이전트 수와 어긋나지 않습니다.",
          ],
        },
        {
          version: "0.3.32",
