      ...
      entries: [
        {
          version: "0.3.31",
          date: "2026-06-26",
          title: "워크스페이스 간 미확인 알림, Composio 툴킷 기반, 더 편한 에디터",
          changes: [],
          features: [
            "워크스페이스 전환기에서 현재 워크스페이스가 아닌 다른 워크스페이스에 미확인 인박스가 있으면 브랜드 색상의 점이 표시됩니다. 워크스페이스를 옮기지 않아도 다른 쪽의 새 활동을 바로 알 수 있고, 계정 단위로 한 번에 집계하기 때문에 알림이 도착하거나 정리되면 점이 실시간으로 켜지고 꺼집니다.",
            "Multica가 독립 실행형 Composio SDK 기반을 갖추어, 곧 도입될 서드파티 툴킷 연동의 발판이 마련됐습니다. 연결 계정 링크, 툴 실행, 서명된 웹훅 수신까지 내부적으로 모두 연결돼 있습니다.",
            "여러 Git 워크트리에서 데스크톱 개발 환경을 동시에 띄워도 충돌이 없습니다. 워크트리마다 별도의 렌더러 포트와 독립된 앱 인스턴스를 받아, 두 체크아웃이 같은 창을 두고 다투지 않습니다.",
            "중문 문서 홈에 클릭하면 재생되는 한어 소개 영상이 추가되었고, 새로 만든 VideoEmbed 빌딩 블록은 다른 문서 페이지에서도 그대로 재사용할 수 있습니다.",
          ],
          improvements: [
            "기여자 문서가 데스크톱 개발 명령이 워크트리별로 자동 격리된다는 점을 분명히 안내하도록 갱신되었습니다.",
          ],
          fixes: [
            "Issue 에디터의 목록에서 Tab을 누르면 선택한 항목이 안정적으로 들여쓰기되고, 커서가 목록 밖으로 빠지지 않습니다. 여러 항목을 선택해 Tab을 누르면 첫 항목을 기준으로 나머지가 그 아래로 중첩되며, 더 이상 들여쓰기할 수 없을 때도 Tab이 편집기 밖으로 포커스를 끌어내지 않습니다.",
            "댓글에서 @멘션이나 답글로 스쿼드 리더에게 작업을 맡기면, 리더가 전체 스쿼드 브리핑을 갖고 시작하므로 혼자 처리하려 들지 않습니다. 부모 댓글의 @스쿼드 멘션을 그대로 이어받은 답글이 리더 실행을 한 번 더 트리거하지도 않습니다.",
            "Issue와 댓글의 코드 블록에서 선택한 텍스트가 페이지의 다른 영역이 다시 렌더링되는 동안에도 풀리지 않으므로, 에이전트 출력이 실시간으로 흐르는 중에도 스니펫을 그대로 복사할 수 있습니다.",
            "Issue를 특정 에이전트에게 바로 할당할 때, 실행 확인 창의 핸드오프 메모 영역이 「확인 중…」 상태에 멈추지 않고 곧바로 열립니다.",
            "Issue 인박스의 미확인 표시가 실제 인박스 화면과 일치하도록 정리되었습니다. 같은 Issue의 중복 알림을 먼저 합친 뒤 세기 때문에, 비어 있는 인박스에 전환기 점이 남아 있는 일이 없습니다.",
            "Issue 댓글 편집 시 저장 버튼에 명확한 로딩 상태가 표시되어, 변경 사항이 실제로 반영될 때까지 시각적으로 알려 줍니다.",
            "백엔드 API 응답 형식이 바뀐 뒤에도 검색 결과가 안정적으로 로드됩니다.",
            "자체 호스팅에서 Docker Compose v2가 설치돼 있지 않으면 즉시 의미 있는 안내 메시지로 멈춥니다. 구버전 명령행이 내뱉던 알 수 없는 'unknown shorthand flag' 오류는 더 이상 보이지 않습니다.",
          ],
        },
        {
          version: "0.3.30",
