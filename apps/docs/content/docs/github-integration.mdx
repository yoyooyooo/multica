...
| **Auto-status on merge** | When a PR with close intent transitions to `merged`, every linked issue not already `Done` or `Cancelled` is moved to `Done`, unless its metadata sets `external_pr_completion_policy=record_only`. The status change is timeline-logged with source `github_pr_merged`. |
