...
description: Connect a GitHub App once to mirror issue-linked PRs; eligible leaf child issues move to Done only after close intent is merged and no linked PR remains open or draft.
---

import { Callout } from "fumadocs-ui/components/callout";

Connect a GitHub account or organization once in **Settings → GitHub**. A PR whose branch or title contains an issue identifier (for example `MUL-123`) is **auto-linked** and displayed under **Pull requests**. A bare identifier found only in the body is retained as an audit reference but is hidden from that list. Merging a PR changes an issue to **Done** only under the leaf-child completion rules below.

There is no per-issue setup. Branch/title matches and body references with an adjacent closing keyword create authoritative links. A body-only bare identifier creates an inferred audit reference: it is hidden and never blocks completion.
...
| **Issue sidebar → Pull requests** | Every visible PR link (title, branch, or closing-keyword authority; body-only bare references stay hidden), with title, repo, state (`Open` / `Draft` / `Merged` / `Closed`), and author. Click a row to jump to the PR on GitHub. |
...
| **Auto-status on merge** | A merged PR with adjacent `Closes` / `Fixes` / `Resolves` intent can complete an eligible leaf child only when no GitHub, native VCS, or external-provider link remains `open` or `draft`. `record_only` records the fact without changing status. |
...
You can reference **multiple issues** in one PR. `Closes MUL-1, MUL-2` links both; each issue is evaluated independently against the leaf-child, policy, and no-open-or-draft-sibling rules.

## The auto-merge-to-Done rule

When an authoritative PR is merged or closed, every linked issue is re-evaluated against the aggregate provider facts. A merge can supply the required close intent; a close can remove the final blocker:
...
| Any non-terminal status with `external_pr_completion_policy=record_only` | The merged PR and issue link remain mirrored, but the issue status does not change. A workflow with an additional completion gate must move it explicitly later. |
| Eligible leaf child; no child issues; at least one authoritative provider link has a merged close intent; no authoritative GitHub, native VCS, or external-provider link remains `open` or `draft`; policy is absent, exact empty, or exact `leaf_child_only` | Moved to `done`. |
| Parent, root issue, issue with children, unsupported policy value, or any authoritative link still `open` / `draft` | **No change.** Inferred body-only references have no blocking authority. |

Closing a PR **without** merging it updates that PR card and re-evaluates the aggregate. If another authoritative link already supplies merged close intent and this close removes the last authoritative `open` / `draft` blocker, an otherwise eligible issue can move to `done`; without that merged enabler, the issue stays where it was.
...
- **Identifiers in commit messages** — only branch / title / body are scanned. A commit titled `MUL-123: fix login` does not auto-link unless the same string also appears in the PR branch or title, or beside a closing keyword in the body.
...
- **Manually linking a PR to an issue** — there is no UI for this yet. Put the identifier in the PR branch or title, or beside `Closes` / `Fixes` / `Resolves` in the body, to create an authoritative link.
...
After that, open a PR whose branch or title contains an issue identifier, or whose body places it beside `Closes` / `Fixes` / `Resolves` — within a few seconds the Pull requests block appears on that issue's detail page. A body-only bare identifier remains an inferred hidden audit reference.
...
- **No manual link UI yet** — authoritative links come from the branch, title, or an adjacent closing keyword in the body; a body-only bare identifier is inferred and hidden.
...
- **Completion policy is metadata-driven** — only an absent value, exact empty string, or exact `leaf_child_only` permits provider completion; exact `record_only` records facts, and all other values fail closed.
- **Multi-PR-to-one-issue completion waits** — an eligible leaf child is not completed while any authoritative GitHub, native VCS, or external-provider link remains `open` or `draft`. Inferred links do not block.
