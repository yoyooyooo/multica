...
<Callout>
If you prefer running the Docker Compose steps manually: `cp .env.example .env`, edit `JWT_SECRET`, `POSTGRES_PASSWORD`, and the password segment in `DATABASE_URL`. Run `docker compose -f docker-compose.selfhost.yml config >/dev/null` before `pull` and `up -d`.
</Callout>
