...
<Callout>
If you prefer running the Docker Compose steps manually: `cp .env.example .env`, edit `JWT_SECRET`, `POSTGRES_PASSWORD`, the password segment in `DATABASE_URL`, and set two stable distinct values: deployment-unique JWT `MULTICA_WORKLOAD_ASSERTION_ISSUER=urn:multica:deployment:<stable-instance-id>` plus secret-free `MULTICA_WORKLOAD_ASSERTION_ISSUER_INSTANCE_ID=<ags-trusted-issuer-id>` that exactly matches AGS `trusted_issuers[].id` and is not inferred from the target. Run `bash scripts/validate-workload-issuer.sh .env` and `docker compose -f docker-compose.selfhost.yml config >/dev/null` before `pull` and `up -d`.
</Callout>
