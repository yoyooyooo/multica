...
cd multica
git pull
# 기존 배포는 안정적이고 서로 다른 identity 값 2개를 .env에 한 번 추가합니다.
# deployment-unique JWT iss:
# MULTICA_WORKLOAD_ASSERTION_ISSUER=urn:multica:deployment:<stable-instance-id>
# secret-free authority linkage(AGS trusted_issuers[].id와 정확히 일치, target에서 추측 금지):
...
HTTP 200이면서 세 검사가 모두 `ok`가 아니라면 새 버전이 migration을 마치지 못한 것입니다. 트래픽을 보내기 전에 백엔드 로그를 확인하세요.
