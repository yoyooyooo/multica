...
| **Merge 自动改 status** | 带 close intent 的 PR 转 `merged` 时，所有已关联且状态不是 `Done` / `Cancelled` 的 issue 会被推到 `Done`；但 metadata 为 `external_pr_completion_policy=record_only` 时除外。时间线里以 source 为 `github_pr_merged` 记录。 |
