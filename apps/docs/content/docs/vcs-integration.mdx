...
- **Issue links** — identifiers in the title, body, or branch link the PR to the issue. A closing keyword (`Closes`/`Fixes`/`Resolves MUL-123`) on a merged PR moves the issue to **Done** once no linked PR is still open. If the issue metadata sets `external_pr_completion_policy=record_only`, the provider PR and link remain mirrored but the issue status and parent Stage stay active until an explicit terminal transition.
