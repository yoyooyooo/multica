...
无论开关如何，工作区目录下的 `{workDir}/.claude/skills/` 都会照常加载——这个开关只控制本机用户级 skill 目录。目前只有 Claude 运行时会读取这个开关：Codex 已经通过 `CODEX_HOME` 做了隔离，ACP 后端（Hermes / Kimi / Kiro）和 JSON-stream 后端（Copilot / Cursor / Gemini / Pi / OpenCode / OpenClaw）都把 skill 发现路径锚在任务工作目录，不会读取本机用户级 skill 目录，所以这些 runtime 对开关是 no-op。
