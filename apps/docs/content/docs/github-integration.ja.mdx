...
| **マージ時のステータス自動変更** | close intent を持つ PR が `merged` に遷移すると、まだ `Done` でも `Cancelled` でもない、紐づいたすべてのイシューが `Done` に移動します。ただし metadata が `external_pr_completion_policy=record_only` の場合を除きます。ステータス変更は source `github_pr_merged` でタイムラインに記録されます。 |
