...
cd multica
git pull
# 旧部署需在 .env 中一次性补齐两个稳定且互不相同的身份值。
# deployment-unique JWT iss：
# MULTICA_WORKLOAD_ASSERTION_ISSUER=urn:multica:deployment:<stable-instance-id>
# secret-free authority linkage（精确等于 AGS trusted_issuers[].id，不得从 target 猜测）：
...
只要不是 HTTP 200 且三项都是 `ok`，就说明新版本没把 migration 跑完，先去看 backend 日志，别急着放流量进来。
