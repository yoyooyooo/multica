...
Anything other than HTTP 200 with both the `db` and `migrations` checks `ok` means the new version did not finish migrating — check the backend logs before sending traffic at it.
