...
cd multica
git pull
# Existing deployments must add both stable, distinct identity values to .env once.
# JWT iss (deployment-unique):
# MULTICA_WORKLOAD_ASSERTION_ISSUER=urn:multica:deployment:<stable-instance-id>
# Secret-free authority linkage (exact AGS trusted_issuers[].id; do not infer from target):
...
Anything other than HTTP 200 with all three checks `ok` means the new version did not finish migrating — check the backend logs before sending traffic at it.
