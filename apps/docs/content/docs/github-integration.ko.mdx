...
| **머지 시 상태 자동 변경** | close intent가 있는 PR이 `merged`로 전환되면, 아직 `Done`이나 `Cancelled`가 아닌 모든 연결된 이슈가 `Done`으로 이동합니다. 단, metadata가 `external_pr_completion_policy=record_only`인 경우는 제외됩니다. 상태 변경은 source `github_pr_merged`로 타임라인에 기록됩니다. |
