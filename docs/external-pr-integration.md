...
4. `external_pr_completion_policy` key 缺省，或 JSON string 经 lowercase + ASCII `space/tab/newline/carriage-return/vertical-tab/form-feed` trim 后为空 / `leaf_child_only`；`record_only`、未知 string、JSON `null` 与非 string 值均 fail closed。
