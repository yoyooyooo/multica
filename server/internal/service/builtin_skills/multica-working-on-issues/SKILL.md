...
provider merge facts without provider-driven terminal completion. Removing it, setting the exact empty string, or setting exact
`leaf_child_only` restores normal close-intent auto-completion for future events;
no other value does.
