...
`external_pr_completion_policy=record_only`, provider PR events still update the
PR mirror and link row, but they never auto-advance the issue to `done`. A child
therefore cannot close its Stage barrier or wake its parent from the provider
merge alone. The workflow that owns the additional completion gate must perform
an explicit terminal transition after its independent evidence is accepted.
Missing policy (or any other value) retains the normal close-intent behavior.
...
- **`done`** on a child issue posts a system comment on its parent. If a PR
  ...
  exception is `external_pr_completion_policy=record_only`: the merge is
  recorded, but the issue and Stage remain active until an explicit terminal
  transition.
