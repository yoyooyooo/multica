...
func TestWorkCoordinationBuiltinSkillBundle(t *testing.T) {
	...
	for _, required := range []string{"name: multica-work-coordination", "multica coordination scope ensure", "multica coordination scope get", "multica coordination dependency add", "multica coordination dependency list", "multica coordination dependency resolve", "Work coordination is passive", "does not own scheduling", "downstream blocked_by upstream"} {
