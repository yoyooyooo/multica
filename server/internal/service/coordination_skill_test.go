...
func TestWorkCoordinationBuiltinSkillBundle(t *testing.T) {
	...
	for _, required := range []string{"name: multica-work-coordination", "multica coordination scope ensure", "multica coordination scope get", "multica coordination dependency add", "multica coordination dependency list", "multica coordination dependency resolve", "multica coordination blocker add", "multica coordination blocker list", "multica coordination blocker resolve", "Work coordination is passive", "does not own scheduling", "downstream blocked_by upstream", "fresh-key duplicate add", "1,000 active dependencies", "1,000 open blockers", "waiting_on_issue", "no_longer_blocking", "blocker resolution never resolves", "mutation revision conflict", "Agent list returns only active pairs containing that task issue"} {
