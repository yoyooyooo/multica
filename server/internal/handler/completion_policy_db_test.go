...
func TestUpdateIssueSerializedPreservesDescriptionAndCompletionLocks(t *testing.T) {
	body := map[string]any{"title": "combined-update", "description": "before", "status": "todo", "priority": "medium"}
	created := httptest.NewRecorder()
	testHandler.CreateIssue(created, newRequest(http.MethodPost, "/api/issues?workspace_id="+testWorkspaceID, body))
	...
	testHandler.UpdateIssue(updated, withURLParam(newRequest(http.MethodPut, "/api/issues/"+issue.ID, map[string]any{
		"description": "after",
		"status":      "done",
	}), "id", issue.ID))
	if updated.Code != http.StatusOK {
		t.Fatalf("combined update status=%d body=%s", updated.Code, updated.Body.String())
	}
	var got IssueResponse
	if err := json.NewDecoder(updated.Body).Decode(&got); err != nil {
		t.Fatal(err)
	}
	if got.Status != "done" || got.Description == nil || *got.Description != "after" {
		t.Fatalf("combined update got status=%q description=%v", got.Status, got.Description)
	}
}
