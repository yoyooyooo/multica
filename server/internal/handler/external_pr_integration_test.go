...
func TestExternalPRProjectionDriftSupersedesMergeDelegationWithEvent(t *testing.T) {
	...
	// A producer that incorrectly reuses the facts revision must still atomically
	// supersede authority when any exact fact drifts.
...
func TestExternalPRSameRevisionExactFactDriftSupersedesMergeDelegation(t *testing.T) {
	ctx := context.Background()
	t.Setenv("MULTICA_EXTERNAL_PR_SERVICE_INSTANCE_ID", "mini")
	mutations := []struct {
		name   string
		...
		{"base", func(p *externalPullRequestLinkRequest) {
			p.ExpectedBaseSHA = "3333333333333333333333333333333333333333"
		}},
		{"binding", func(p *externalPullRequestLinkRequest) {
			p.ProviderBindingRevision = "sha256:ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
		}},
		{"method", func(p *externalPullRequestLinkRequest) { p.DelegatedMergeMethod = "squash" }},
		{"canonical facts", func(p *externalPullRequestLinkRequest) {
			p.ProviderRepository = "ux/smip-v2"
			p.MergeRepo = "ux/smip-v2"
		}},
	}
	for index, test := range mutations {
		t.Run(test.name, func(t *testing.T) {
			issueID := createExternalPRTestIssue(t, "same revision "+test.name, "in_review", "", nil)
			externalNumber := int32(70 + index*2)
			mergeNumber := externalNumber + 1
			intent := false
			projection := externalPullRequestLinkRequest{
				Provider: "ags", IssueID: issueID, WorkspaceID: testWorkspaceID, Workspace: handlerTestWorkspaceSlug,
				IssueKey: "HAN-same-revision", ExternalRepo: "ux/smip", ExternalNumber: externalNumber,
				MergeProvider: "forgejo", MergeRepo: "ux/smip", MergeNumber: mergeNumber,
				LinkConfidence: "authoritative", CompletionIntent: &intent, State: "open", IdempotencyKey: "same-revision-v1-" + issueID,
				TargetInstance: "mini", CanonicalRepositoryID: "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
				CanonicalRepository: "ux/smip", ProviderBindingID: "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
				ProviderBindingRevision: "sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
				ProviderRepository:      "ux/smip", ExpectedHeadSHA: "1111111111111111111111111111111111111111",
			...
			var linkID, delegationID string
			if err := testPool.QueryRow(ctx, `SELECT id FROM external_pull_request_link WHERE workspace_id=$1 AND issue_id=$2 AND external_number=$3`, testWorkspaceID, issueID, externalNumber).Scan(&linkID); err != nil {
				t.Fatal(err)
			}
			if err := testPool.QueryRow(ctx, `INSERT INTO workload_pr_merge_delegation (
				workspace_id,issue_id,external_pr_link_id,task_id,execution_id,runtime_id,
				target_instance,canonical_repository_id,canonical_repository,provider_binding_id,provider_binding_revision,
				provider_repository,ags_pr_number,provider_pr_number,expected_head_sha,expected_base_sha,base_ref,merge_method,
				projection_facts_revision,facts_digest,state
			) VALUES ($1,$2,$3,$4,$5,$6,'mini',$7,'ux/smip',$8,$9,'ux/smip',$10,$11,$12,$13,'main','rebase',$14,$15,'pending_approval') RETURNING id`,
				testWorkspaceID, issueID, linkID, uuid.NewString(), uuid.NewString(), uuid.NewString(),
				projection.CanonicalRepositoryID, projection.ProviderBindingID, projection.ProviderBindingRevision,
				externalNumber, mergeNumber, projection.ExpectedHeadSHA, projection.ExpectedBaseSHA, projection.ProjectionFactsRevision,
				"sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee").Scan(&delegationID); err != nil {
				t.Fatalf("seed delegation: %v", err)
			}
			t.Cleanup(func() {
			...
			projection.IdempotencyKey = "same-revision-v2-" + issueID
			if _, err := testHandler.upsertExternalPullRequestLink(ctx, projection); err != nil {
				t.Fatalf("update projection: %v", err)
			}
			var state string
			...
			if err := testPool.QueryRow(ctx, `SELECT count(*) FROM workload_pr_merge_delegation_event WHERE delegation_id=$1 AND event_type='superseded'`, delegationID).Scan(&events); err != nil || events != 1 {
