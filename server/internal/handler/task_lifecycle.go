...
// RerunIssueRequest is the body shared by the execution-log retry endpoint and
// the dedicated fresh-provenance endpoint.
type RerunIssueRequest struct {
	// TaskID identifies the exact execution row whose actor, squad role, and
	// trigger-comment provenance should be preserved.
	TaskID string `json:"task_id,omitempty"`
}

// RerunIssue preserves the existing execution-log retry semantics. An empty
// body targets the current assignee and starts clean; task_id targets the exact
// historical actor and may reuse that source task's workdir/session.
func (h *Handler) RerunIssue(w http.ResponseWriter, r *http.Request) {
	h.rerunIssue(w, r, false)
}

// RerunIssueFresh requires task_id and preserves only source provenance. It
// creates a task that cannot reuse the source session or workdir, including
// when an older claim handler processes it during a rolling deploy.
func (h *Handler) RerunIssueFresh(w http.ResponseWriter, r *http.Request) {
	h.rerunIssue(w, r, true)
}

func (h *Handler) rerunIssue(w http.ResponseWriter, r *http.Request, freshContext bool) {
	...
	if err != nil {
		slog.Warn("issue rerun failed", "issue_id", id, "fresh_context", freshContext, "error", err)
