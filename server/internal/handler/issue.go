...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	if req.Status != nil || parentTouched {
		...
		var current db.Issue
		issue, current, err = h.updateIssueWithDescriptionMerge(r.Context(), prevIssue.WorkspaceID, params, rawFields, req.DescriptionBase)
		if err == nil {
			prevIssue = current
		}
	} else {
...
func (h *Handler) deleteIssueAndCollectAttachmentURLs(ctx context.Context, issue db.Issue) ([]string, error) {
	...
	if err := qtx.FailAutopilotRunsByIssue(ctx, issue.ID); err != nil {
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	for _, issueID := range req.IssueIDs {
		...
		if req.Updates.Status != nil || parentTouched {
			...
			var current db.Issue
			issue, current, err = h.updateIssueWithDescriptionMerge(r.Context(), prevIssue.WorkspaceID, params, rawUpdates, nil)
			if err == nil {
				prevIssue = current
			}
		} else {
