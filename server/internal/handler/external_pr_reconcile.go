...
func (h *Handler) processExternalPRWork(ctx context.Context, work externalPRWork) error {
	...
	terminal, err := h.childStatusResolver(ctx)(issue)
	if err != nil {
		return err
	}
...
SELECT EXISTS (
    ...
    WHERE link.issue_id=$1 AND pr.state IN ('open','draft')
    UNION ALL
...
func (h *Handler) finalizeExternalPRParent(ctx context.Context, work externalPRWork, issue db.Issue) (pgtype.UUID, error) {
	...
	previousStatus, err := effective(previous)
	if err != nil {
		return pgtype.UUID{}, err
	}
	currentStatus, err := effective(issue)
	if err != nil {
		return pgtype.UUID{}, err
	}
	if isTerminalChildStatus(previousStatus) || !isTerminalChildStatus(currentStatus) {
		return pgtype.UUID{}, nil
	}
	...
	comment, err := h.postChildDoneCommentWithID(ctx, parent, issue, children, staged, closedStage, false, statuses, nil, work.ID)
	if err != nil {
		return pgtype.UUID{}, err
	}
