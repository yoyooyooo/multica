...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	case ctx.ChatSessionID != "":
		b.WriteString("This is a chat task. Your final assistant response is delivered directly to the chat window, so do not call `multica issue comment add` unless the user explicitly asks you to write a comment to a specific issue.\n")
	default:
		if ctx.IsSquadLeader {
