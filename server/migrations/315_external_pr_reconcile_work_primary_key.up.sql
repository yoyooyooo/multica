-- Attach the exact id authority from migration 296. The conditional block is
...
DO $$
DECLARE
    table_oid oid := to_regclass('external_pr_reconcile_work');
    source_index_oid oid := to_regclass('external_pr_reconcile_work_id_uidx');
...
BEGIN
    IF table_oid IS NULL THEN
        RAISE EXCEPTION 'external_pr_reconcile_work table is missing; refuse primary-key recovery';
    END IF;
    ...
    SELECT c.oid, c.conindid, idx.relname
     ...
     WHERE c.conrelid = table_oid
       AND c.contype = 'p'
       AND c.conname = 'external_pr_reconcile_work_pkey';

    IF primary_key_count > 1 OR (primary_key_count = 1 AND target_constraint_oid IS NULL) THEN
        RAISE EXCEPTION 'external_pr_reconcile_work has an unexpected primary-key authority; refuse recovery';
    END IF;
    ...
    IF target_constraint_oid IS NOT NULL THEN
        SELECT EXISTS (
            SELECT 1
             ...
             WHERE c.oid = target_constraint_oid
               ...
               AND idx.relname = 'external_pr_reconcile_work_pkey'
               AND idx.relkind = 'i'
               AND am.amname = 'btree'
        ...
        IF NOT target_exact OR target_index_oid IS NULL THEN
            RAISE EXCEPTION 'external_pr_reconcile_work primary-key authority is not exact; refuse recovery';
        END IF;
        IF source_index_oid IS NOT NULL THEN
            RAISE EXCEPTION 'external_pr_reconcile_work has both attached and source index authorities; refuse recovery';
        END IF;
        RETURN;
    END IF;

    IF source_index_oid IS NULL THEN
        RAISE EXCEPTION 'external_pr_reconcile_work id authority is missing; refuse primary-key attach';
    END IF;
    ...
    IF source_constraint_count <> 0 THEN
        RAISE EXCEPTION 'external_pr_reconcile_work source index is already constraint-owned; refuse recovery';
    END IF;
    ...
    IF NOT source_exact THEN
        RAISE EXCEPTION 'external_pr_reconcile_work source id index is not exact; refuse primary-key attach';
    END IF;

    ALTER TABLE external_pr_reconcile_work
        ADD CONSTRAINT external_pr_reconcile_work_pkey
        PRIMARY KEY USING INDEX external_pr_reconcile_work_id_uidx;
END
$$;
