...
const contextContinuityTestDB = "multica_cc_v049"
...
func TestContextContinuityMigrationConvergence(t *testing.T) {
	...
	for _, scenario := range []string{"clean_main_223", "historical_135", "fork_v0_4_8"} {
...
func testContextContinuityMigrationScenario(t *testing.T, dbURL, scenario string) {
	...
	schema := fmt.Sprintf("cc_v049_%s_%d_%d", scenario, time.Now().UnixNano(), rand.Uint32())
	...
	case "historical_135":
		...
		run(realMigrationRange(t, serverRoot, 135, 223))
		wantLegacyRow = true
	case "fork_v0_4_8":
		...
		seedCurrentBaseRows(t, ctx, pool)
		seedLegacyExternalPRRow(t, ctx, pool)
		run(realMigrationRange(t, serverRoot, 212, 223))
		wantLegacyRow = true
	default:
	...
	// mutation after 229 and before 230/231 must be retained by the temporary
	// trigger installed while the member table was still locked.
	run(realMigrationRange(t, serverRoot, 224, 229))
...
WHERE workspace_id='00000000-0000-4000-8000-000000000049'`).Scan(&epochBefore); err != nil {
	...
	if _, err := pool.Exec(ctx, `UPDATE member SET role='admin' WHERE id='00000000-0000-4000-8000-000000000249'`); err != nil {
		t.Fatalf("mutate member between 229 and 230: %v", err)
	}
...
WHERE workspace_id='00000000-0000-4000-8000-000000000049'`).Scan(&epochAfterTemporaryTrigger); err != nil {
	...
	run(realMigrationRange(t, serverRoot, 230, 231))
	...
	// path while migration 228's receipt authority (and legacy authority on old
	// origins) remains valid. The failed migration must not be ledgered.
	migration232 := realMigrationRange(t, serverRoot, 232, 232)
	exerciseMigration232Recovery(t, ctx, pool, tableFQN, tableName, lockKey, migration232, scenario)
	run(migration232)
	...
	exerciseMigration233Gate(t, ctx, pool, serverRoot, tableName, lockKey)
	run(realMigrationRange(t, serverRoot, 233, 233))
	prepareReconciliationFenceRecovery(t, ctx, pool)
	run(realMigrationRange(t, serverRoot, 234, 234))
	...
	run(realMigrationRange(t, serverRoot, 235, 236))
...
func exerciseCleanupIndexInvalidRecovery(
	...
	tests := []struct {
		...
		{235, "235_external_pr_link_issue_updated_index", externalPRCleanupIndexSpecs[0]},
		{236, "236_external_pr_receipt_issue_cleanup_index", externalPRCleanupIndexSpecs[1]},
	}
...
func exerciseMigration232Recovery(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	...
	t.Helper()
	const version = "232_external_pr_link_workspace_idempotency_index"

	if scenario == "clean_main_223" {
    ...
    ('00000000-0000-4000-8000-000000000049', '00000000-0000-4000-8000-000000000149',
     'ags', 'jackie/invalid-index-two', 4702, 'authoritative', TRUE, 'open', $1)`, duplicateKey); err != nil {
		...
		err := runMigrations(ctx, pool, runOptions{
			Direction: "up", Files: migration232, SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		})
		if err == nil {
			t.Fatal("migration 232 unexpectedly accepted duplicate workspace keys")
		}
...
WHERE c.oid=to_regclass('idx_external_pr_link_workspace_idempotency')`).Scan(&ready, &valid); scanErr != nil {
	...
	if err := pool.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM "+tableFQN+" WHERE version=$1)", version).Scan(&ledgered); err != nil {
		t.Fatalf("read migration 232 ledger after failed apply: %v", err)
	}
	if scenario == "clean_main_223" && ledgered {
		t.Fatal("failed concurrent migration 232 was ledgered")
	}
	if scenario != "clean_main_223" && !ledgered {
		t.Fatal("historical wrong-definition migration 232 fixture was not ledgered")
	}
}
...
func exerciseLedgeredIndexRecovery(
	...
	versions := []struct {
		...
		{225, "225_external_pr_link_id_unique_index", externalPRIndexSpecs[0]},
		{226, "226_external_pr_link_identity_index", externalPRIndexSpecs[1]},
		{227, "227_external_pr_link_issue_state_index", externalPRIndexSpecs[2]},
		{228, "228_external_pr_link_idempotency_index", externalPRIndexSpecs[3]},
		{230, "230_workspace_workload_authority_workspace_id_index", externalPRIndexSpecs[4]},
		{232, "232_external_pr_link_workspace_idempotency_index", externalPRIndexSpecs[5]},
	}
	...
	// Simulate the historical 232 IF NOT EXISTS bug exactly on the clean-main
	// origin, where no legacy global index blocks the duplicate fixture. The
	// other origins already exercise ledgered wrong definitions while retaining
	// the legacy authority until 233.
	...
	const duplicateKey = "cc-v049-ledgered-invalid"
	if _, err := pool.Exec(ctx, `
INSERT INTO external_pull_request_link (
    workspace_id, issue_id, provider, external_repo, external_number,
    link_confidence, completion_intent, state, idempotency_key
	...
	if err := runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 232, 232), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	}); err != nil {
		t.Fatalf("repair ledgered invalid 232 index: %v", err)
	}
...
func exerciseMigration233Gate(t *testing.T, ctx context.Context, pool *pgxpool.Pool, serverRoot, tableName string, lockKey int64) {
	t.Helper()
	spec := externalPRIndexSpecs[5]
	schema, _, _, err := inspectMigrationIndex(ctx, pool, spec)
	if err != nil {
	...
	err = runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 233, 233), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	})
	if err == nil || !strings.Contains(err.Error(), "refuse legacy idempotency index removal") {
		t.Fatalf("migration 233 did not fail closed on wrong 232 authority: %v", err)
	}
	if repairErr := runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 232, 232), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	}); repairErr != nil {
		t.Fatalf("repair 232 after 233 gate: %v", repairErr)
	}
}
...
func assertReconciliationFenceStopsHistoricalRepair(t *testing.T, ctx context.Context, pool *pgxpool.Pool, serverRoot, tableName string, lockKey int64) {
	...
	downFence := filepath.Join(serverRoot, "migrations", "234_external_pr_index_reconciliation_fence.down.sql")
	err = runMigrations(ctx, pool, runOptions{
	...
	if !fenceLedgered {
		t.Fatal("rejected down removed migration 234 ledger")
	}
	if err := runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 225, 234), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	}); err != nil {
...
WHERE n.nspname=$1 AND c.relname='idx_external_pr_link_workspace_idempotency'`, schema).Scan(&ready, &valid); err != nil {
		t.Fatalf("workspace idempotency authority missing after 232 in %s: %v", scenario, err)
	}
	if !ready || !valid {
		t.Fatalf("workspace idempotency authority after 232 in %s ready=%v valid=%v", scenario, ready, valid)
	}
...
INSERT INTO external_pull_request_link (
    ...
    ('00000000-0000-4000-8000-000000000049', '00000000-0000-4000-8000-000000000149',
     'ags', 'jackie/intermediate-two', 492, 'authoritative', TRUE, 'open', $1)`, key); err == nil {
		t.Fatalf("same-workspace duplicate key was accepted after 232 in %s", scenario)
	}
}
