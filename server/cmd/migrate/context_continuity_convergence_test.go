...
const contextContinuityTestDB = "multica_cc_v0412"
...
func testContextContinuityMigrationScenario(t *testing.T, dbURL, scenario string) {
	...
	schema := fmt.Sprintf("cc_v0412_%s_%d_%d", scenario, time.Now().UnixNano(), rand.Uint32())
	...
	switch scenario {
	case "clean_v0_4_12":
		run(realMigrationRange(t, serverRoot, 1, 230))
	...
	case "historical_135":
		...
		run(realMigrationRange(t, serverRoot, 135, 230))
		wantLegacyRow = true
	case "fork_v0_4_8":
		...
		seedCurrentBaseRows(t, ctx, pool)
		seedLegacyExternalPRRow(t, ctx, pool)
		run(realMigrationRange(t, serverRoot, 212, 230))
		wantLegacyRow = true
	case "fork_v0_4_9":
		...
		// The deployed v0.4.9 ledger already owns its old 224-236 migration
		// names. Apply the official v0.4.12 224-230 migrations afterward, then
		// prove the newly numbered 231-243 reconciliation is idempotent.
		run(realMigrationRange(t, serverRoot, 224, 230))
	default:
		t.Fatalf("unknown scenario %q", scenario)
	}

	// Split the reconciliation at the concurrent index boundary. A membership
	// mutation after 236 and before 237/238 must be retained by the temporary
	// trigger installed while the member table was still locked.
	run(realMigrationRange(t, serverRoot, 231, 236))
...
WHERE workspace_id='00000000-0000-4000-8000-000000000049'`).Scan(&epochBefore); err != nil {
	...
	if _, err := pool.Exec(ctx, `UPDATE member SET role='admin' WHERE id='00000000-0000-4000-8000-000000000249'`); err != nil {
		t.Fatalf("mutate member between 236 and 237: %v", err)
	}
...
WHERE workspace_id='00000000-0000-4000-8000-000000000049'`).Scan(&epochAfterTemporaryTrigger); err != nil {
	...
	run(realMigrationRange(t, serverRoot, 237, 238))
	...
	// path while migration 235's receipt authority (and legacy authority on old
	// origins) remains valid. The failed migration must not be ledgered.
	migration239 := realMigrationRange(t, serverRoot, 239, 239)
	exerciseMigration239Recovery(t, ctx, pool, tableFQN, tableName, lockKey, migration239, scenario)
	run(migration239)
	...
	exerciseMigration240Gate(t, ctx, pool, serverRoot, tableName, lockKey)
	run(realMigrationRange(t, serverRoot, 240, 240))
	prepareReconciliationFenceRecovery(t, ctx, pool)
	run(realMigrationRange(t, serverRoot, 241, 241))
	...
	run(realMigrationRange(t, serverRoot, 242, 243))
...
func exerciseCleanupIndexInvalidRecovery(
	...
	tests := []struct {
		...
		{242, "242_external_pr_link_issue_updated_index", externalPRCleanupIndexSpecs[0]},
		{243, "243_external_pr_receipt_issue_cleanup_index", externalPRCleanupIndexSpecs[1]},
	}
...
func exerciseMigration239Recovery(
	t *testing.T,
	ctx context.Context,
	pool *pgxpool.Pool,
	...
	t.Helper()
	const version = "239_external_pr_link_workspace_idempotency_index"
    ...
    ('00000000-0000-4000-8000-000000000049', '00000000-0000-4000-8000-000000000149',
     'ags', 'jackie/invalid-index-two', 4702, 'authoritative', TRUE, 'open', $1)`, duplicateKey); err != nil {
		...
		err := runMigrations(ctx, pool, runOptions{
			Direction: "up", Files: migration239, SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		})
		if err == nil {
			t.Fatal("migration 239 unexpectedly accepted duplicate workspace keys")
		}
...
WHERE c.oid=to_regclass('idx_external_pr_link_workspace_idempotency')`).Scan(&ready, &valid); scanErr != nil {
	...
	if err := pool.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM "+tableFQN+" WHERE version=$1)", version).Scan(&ledgered); err != nil {
		t.Fatalf("read migration 239 ledger after failed apply: %v", err)
	}
	if scenario == "clean_v0_4_12" && ledgered {
		t.Fatal("failed concurrent migration 239 was ledgered")
	}
	if scenario != "clean_v0_4_12" && !ledgered {
		t.Fatal("historical wrong-definition migration 239 fixture was not ledgered")
	}
}
...
func exerciseLedgeredIndexRecovery(
	...
	versions := []struct {
		...
		{232, "232_external_pr_link_id_unique_index", externalPRIndexSpecs[0]},
		{233, "233_external_pr_link_identity_index", externalPRIndexSpecs[1]},
		{234, "234_external_pr_link_issue_state_index", externalPRIndexSpecs[2]},
		{235, "235_external_pr_link_idempotency_index", externalPRIndexSpecs[3]},
		{237, "237_workspace_workload_authority_workspace_id_index", externalPRIndexSpecs[4]},
		{239, "239_external_pr_link_workspace_idempotency_index", externalPRIndexSpecs[5]},
	}
...
INSERT INTO external_pull_request_link (
	...
	if err := runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 239, 239), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	}); err != nil {
		t.Fatalf("repair ledgered invalid 239 index: %v", err)
	}
...
func exerciseMigration240Gate(t *testing.T, ctx context.Context, pool *pgxpool.Pool, serverRoot, tableName string, lockKey int64) {
	t.Helper()
	spec := externalPRIndexSpecs[5]
	schema, _, _, err := inspectMigrationIndex(ctx, pool, spec)
	if err != nil {
	...
	err = runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 240, 240), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	})
	if err == nil || !strings.Contains(err.Error(), "refuse legacy idempotency index removal") {
		t.Fatalf("migration 240 did not fail closed on wrong 239 authority: %v", err)
	}
	if repairErr := runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 239, 239), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	}); repairErr != nil {
		t.Fatalf("repair 239 after 240 gate: %v", repairErr)
	}
}
...
func assertReconciliationFenceStopsHistoricalRepair(t *testing.T, ctx context.Context, pool *pgxpool.Pool, serverRoot, tableName string, lockKey int64) {
	...
	downFence := filepath.Join(serverRoot, "migrations", "241_external_pr_index_reconciliation_fence.down.sql")
	err = runMigrations(ctx, pool, runOptions{
	...
	if !fenceLedgered {
		t.Fatal("rejected down removed migration 241 ledger")
	}
	if err := runMigrations(ctx, pool, runOptions{
		Direction: "up", Files: realMigrationRange(t, serverRoot, 232, 241), SchemaMigrationsTable: tableName, AdvisoryLockKey: lockKey,
		ReconcileHooks: reconciliationMigrationHooks, ReconcileFenceVersion: externalPRIndexReconciliationFenceVersion,
	}); err != nil {
