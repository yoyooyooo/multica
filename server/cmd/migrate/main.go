...
var preMigrationHooks = map[string]preMigrationHook{
	...
	"235_external_pr_link_issue_updated_index":    reconcileMigrationIndex(externalPRCleanupIndexSpecs[0]),
	"236_external_pr_receipt_issue_cleanup_index": reconcileMigrationIndex(externalPRCleanupIndexSpecs[1]),
}

const externalPRIndexReconciliationFenceVersion = "234_external_pr_index_reconciliation_fence"

// reconciliationMigrationHooks run before the ledger skip check only until
// migration 234 records the final catalog fence. Concurrent index creation can
...
var reconciliationMigrationHooks = map[string]preMigrationHook{
	"225_external_pr_link_id_unique_index":                reconcileMigrationIndex(externalPRIndexSpecs[0]),
	"226_external_pr_link_identity_index":                 reconcileMigrationIndex(externalPRIndexSpecs[1]),
	"227_external_pr_link_issue_state_index":              reconcileMigrationIndex(externalPRIndexSpecs[2]),
	"228_external_pr_link_idempotency_index":              reconcileMigrationIndex(externalPRIndexSpecs[3]),
	"230_workspace_workload_authority_workspace_id_index": reconcileMigrationIndex(externalPRIndexSpecs[4]),
	"232_external_pr_link_workspace_idempotency_index":    reconcileMigrationIndex(externalPRIndexSpecs[5]),
	"233_external_pr_legacy_idempotency_index_remove":     verifyExternalPRIndexAuthorities,
